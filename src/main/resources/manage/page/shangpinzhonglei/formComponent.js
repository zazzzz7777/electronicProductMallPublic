Vue.component('shangpinzhonglei-form', {
template:`
<template>
    <div>
        <el-dialog
            ...
            <el-form class="formModel_form" ref="formRef" :model="form" :rules="rules" >
                <el-form-item  label="商品种类" prop="shangpinzhonglei" class="input-item">
                    <el-input v-model="form.shangpinzhonglei"
                       placeholder="商品种类"
                       type="text"
                       :readonly="!isAdd||disabledForm.shangpinzhonglei?true:false" ></el-input>
                </el-form-item>
            </el-form>
...
methods: {
    init(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null){
        ...
        if(formType == 'add'){
            ...
            for(let x in row){
                if(x=='shangpinzhonglei'){
                    this.form.shangpinzhonglei = row[x];
                    this.disabledForm.shangpinzhonglei = true;
                    continue;
                }
            }
            if(row){
    ...
    getInfo(){
        http.get(`shangpinzhonglei/info/${this.id}`).then(res=>{
    ...
    save(){
        ...
        this.$refs.formRef.validate((valid)=>{
            ...
            if(crossUserId&&crossRefId){
                ...
                http.get('shangpinzhonglei/page',{
                    params:params
                }).then(res=>{
                    if(res.data.data.total>=crossOptNum){
                        return this.$message.error(this.crossTips)
                    }else{
                        http.post(`shangpinzhonglei/${!this.form.id ? "save" : "update"}`,this.form).then(res=>{
                            if(this.type == 'cross'){
                                //修改跨表数据
